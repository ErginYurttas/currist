...
const projectIdToOpen = isNewProject
  ? null
  : urlProjectId || savedProjectId;

if (!projectIdToOpen) return;

    try {
      const project =
        await ProjectManager.getCloudProject(projectIdToOpen);

      applyProjectDocument(project);
      ProjectManager.saveProject(project);

      setLastOpenedProjectId(projectIdToOpen);
...
>
  ...
  >
    {buildingType || "Building type not selected"}
    {" · "}
    {projectCountry || "Country not selected"}
  </div>
</div>
